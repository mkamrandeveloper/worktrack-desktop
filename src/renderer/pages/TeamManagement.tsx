    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={loadTeam} className="text-xs text-primary border border-primary/30 rounded-xl px-4 py-2 hover:bg-primary/10 transition-all duration-300 hover:shadow-sm active:scale-95">
            Refresh
          </button>
          <button onClick={() => setShowAddEmployee(true)} className="flex items-center gap-2 text-xs font-semibold text-primary-foreground bg-primary rounded-xl px-4 py-2 hover:bg-primary/90 transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:scale-95">
      ...
      {/* Join Requests */}
      {requests.length > 0 && (
        <div className="glass-panel border border-amber-500/30 rounded-3xl overflow-hidden shadow-sm">
          <div className="bg-amber-500/5 border-b border-amber-500/20 px-6 py-4 flex items-center gap-2">
              ...
              <div key={req.id} className="flex items-center justify-between px-5 py-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-green-500/10 border border-green-500/30 text-green-500 text-xs font-semibold rounded-xl hover:bg-green-500/20 transition-all duration-300 hover:-translate-y-0.5 active:scale-95 disabled:opacity-50"
                  >
                    {actionPending === req.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <UserCheck className="w-3.5 h-3.5" />}
                    Approve
                  </button>
                  <button
                    ...
                    disabled={actionPending === req.id}
                    className="flex items-center gap-1.5 px-4 py-2 bg-destructive/10 border border-destructive/30 text-destructive text-xs font-semibold rounded-xl hover:bg-destructive/20 transition-all duration-300 hover:-translate-y-0.5 active:scale-95 disabled:opacity-50"
                  >
      ...
      {/* Active Members */}
      <div className="glass-panel border border-border rounded-3xl overflow-hidden shadow-sm">
        <div className="border-b border-border px-6 py-4 flex items-center justify-between bg-card/30">
          ...
          <button
            ...
            onClick={() => openDrive()}
            className="flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground border border-border rounded-xl px-4 py-2 hover:bg-muted transition-all duration-300 hover:shadow-sm active:scale-95"
          >
              ...
              <div key={member.id} className="flex items-center justify-between px-5 py-4 hover:bg-muted/30 transition">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs text-muted-foreground border border-border rounded-xl px-4 py-2 hover:bg-muted hover:text-foreground transition-all duration-300 hover:-translate-y-0.5 active:scale-95"
                  >
